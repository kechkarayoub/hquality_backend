...
from .utils import get_user_by_email_or_username, send_new_user_confirmation_email
...
class RegisterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        request.session['language_id'] = user.language
        send_new_user_confirmation_email(user, user_email_confirmation_key.key) if settings.TEST_SETTINGS else send_new_user_confirmation_email.after_response(user, user_email_confirmation_key.key)
        response = JsonResponse({
            'success': True,
            'messages': {
...
class ResendActivationEmailView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        for u_e_c_k in user.my_email_confirmation_keys.filter():
            if u_e_c_k.is_expired:
                pass
            else:
                user_email_confirmation_key = u_e_c_k
                break
